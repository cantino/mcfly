...
impl Default for Weights {
    fn default() -> Weights {
        Weights { offset: 0.0, age: -0.1, exit: -5.0, dir: 1.0, overlap: 2.0, occurrences: 0.2 }
    }
}
...
impl Weights {
    ...
    ///   ((self.offset + age * self.age + exit * self.exit + dir * self.dir + overlap * self.overlap + occurrences * self.occurrences - 1.0)^2
    pub fn online_update(&self, lr: f64, target: f64, age: f64, exit: f64, dir: f64, overlap: f64, occurrences: f64) -> Weights {
        let rank = self.offset + age * self.age + exit * self.exit + dir * self.dir + overlap * self.overlap + occurrences * self.occurrences;
        Weights {
