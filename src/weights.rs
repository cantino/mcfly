...
impl Weights {
    ...
    pub fn online_update(&self, lr: f64, correct: f64, age: f64, exit: f64, recent_failure: f64, dir: f64, overlap: f64, immediate_overlap: f64, occurrences: f64) -> Weights {
        let rank = self.rank(age, exit, recent_failure, dir, overlap, immediate_overlap, occurrences);
        Weights {
    ...
    pub fn rank(&self, age: f64, exit: f64, recent_failure: f64, dir: f64, overlap: f64, immediate_overlap: f64, occurrences: f64) -> f64 {
        self.offset + age * self.age + exit * self.exit + recent_failure * self.recent_failure + dir * self.dir + overlap * self.overlap + immediate_overlap * self.immediate_overlap + occurrences * self.occurrences
    }
}
