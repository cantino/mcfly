...
impl<'a> Trainer<'a> {
    ...
    pub fn train(&mut self) {
        ...
        for _ in 1..1000 {
            ...
            let mut dir_increment = momentum * 0.0;
...

            {
                ...
                generator.generate(200, 5, |command: &Command, _max_occurrences: f64, correct: bool| {
                    ...
                    let prediction = weights.rank(command.age_factor, command.exit_factor, command.recent_failure_factor, command.dir_factor, command.overlap_factor, command.immediate_overlap_factor, command.occurrences_factor);
